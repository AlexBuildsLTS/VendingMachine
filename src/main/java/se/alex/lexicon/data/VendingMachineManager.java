...
public class VendingMachineManager implements VendingMachine {
    ...
    public Product request(int id) {
        ...
        if (requestedProduct != null) {
            if (requestedProduct.getPrice() <= depositPool) {
                ...
                throw new IllegalArgumentException("Insufficient funds for product: " + requestedProduct.getName());
            }
        } else {
    ...
    public String[] getProducts() {
        ...
        for (int i = 0; i < products.length; i++) {
            productsAsString[i] = "ID: " + products[i].getId() + ", Name: " + products[i].getName() + ", Price: " + products[i].getPrice();
        }
